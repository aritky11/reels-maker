...
with st.sidebar.expander("位置・サイズ・行間の微調整", expanded=True):
    st.subheader("① タイトル設定")
    size_title = st.slider("タイトル文字サイズ", 30, 150, 80, key="s_title")
    spacing_title = st.slider("タイトルの行間", 0, 50, 10, key="sp_title")
    y_title = st.slider("タイトル上下位置 (Y)", 50, 500, 160, key="y_title")
    x_title_offset = st.slider("タイトル左右のズレ", -500, 500, 0, key="x_title")

    st.subheader("② 本文設定")
    size_body = st.slider("本文文字サイズ", 20, 100, 45, key="s_body")
    spacing_body = st.slider("本文の行間", 10, 100, 30, key="sp_body")
    y_body_offset = st.slider("本文上下のズレ (中央基準)", -500, 500, 0, key="y_body")
    x_body_offset = st.slider("本文左右のズレ", -500, 500, 0, key="x_body")

    st.subheader("③ フッター設定")
    size_footer = st.slider("フッター文字サイズ", 20, 100, 40, key="s_footer")
    spacing_footer = st.slider("フッターの行間", 0, 50, 10, key="sp_footer")
    y_footer = st.slider("フッター上下位置 (Y)", 1000, 1900, 1650, key="y_footer")
    x_footer_offset = st.slider("フッター左右のズレ", -500, 500, 0, key="x_footer")
...
def draw_text_with_bold(draw, position, text, font, fill, align, spacing, is_bold, strength):
    x, y = position
    if is_bold:
        # 文字の周囲に少しずつずらして重ね書きすることで、明朝体のトゲを潰さずに綺麗に太らせる
...
def create_preview_image():
    # 修正点①: 緑色のbase.png依存を無くし、レイラ仕様の【RGB純黒（#000000）】をコードで自動生成
    ...
    # 3. 本文描画 (修正点②: alignを"center"にし、バウンディングボックスの計算と描画を完全中央配置に最適化)
