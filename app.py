...
# 太さの微調整（ここを小さくするとシュッとします）
bold_thickness = st.sidebar.slider("太字の強度（初期値1.5が推奨）", 0.5, 5.0, 1.5, step=0.1)

# 位置・サイズの微調整（アコーディオン）
with st.sidebar.expander("位置・サイズの微調整（重なる場合はここ）"):
    size_title = st.slider("タイトル文字サイズ", 50, 150, 80) # 初期値を少し下げました
...
with col1:
    ...
    body_input = st.text_area("② 本文", height=350) # 本文は前回と同じため省略表示
...
def create_preview_image():
    ...
    draw_styled_text(draw, ((W-(bbox_t[2]-bbox_t[0]))/2-bbox_t[0], y_title), title_text, font_t, (255,255,255,255), "center", 20, is_bold_title, bold_thickness)
    ...
    # 自動中央配置（重なりを防ぐため、最低限の余白を確保）
    y_b = max(title_bottom + 50, title_bottom + (y_footer - title_bottom - body_h) / 2 - bbox_b[1] + y_body_offset)
    draw_styled_text(draw, ((W-(bbox_b[2]-bbox_b[0]))/2-bbox_b[0], y_b), body_input, font_b, (255,255,255,255), "left", 30, is_bold_body, bold_thickness)
